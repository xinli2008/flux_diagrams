...
class FluxPipeline(
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            # 在flux中，我们使用两个文本编码器：CLIPTextModel和T5EncoderModel。
            # CLIPTextModel用于捕捉图像的全局语义信息，而T5EncoderModel则用于捕捉更细粒度的文本特征。
    ...
    def __call__(
        ...
        # 在Flux中, 我们根据图像的序列长度来动态计算mu的值, 让模型能够更好的处理不同分辨率的图像。
        # Flux模型使用了一种改进的调度策略, 对于不同分辨率的图像, 最优的噪声调动是不同的。
        # 更高的分辨率(也就是更大的image_seq_len)通常需要更大的mu值。
        mu = calculate_shift(
            image_seq_len,
